...
import { DEFAULT_SETTINGS, type NoteArchitectSettings, type FrontmatterPreset } from '@types';
import { FieldConfigModal } from './field-config-modal';
import { CreatePresetModal } from './create-preset-modal';
import {
	renderPresetListUI,
	type RenderPresetItemContext,
} from './preset-item-ui';
...
export class NoteArchitectSettingTab extends PluginSettingTab {
	...
	constructor(app: App, plugin: NoteArchitect, settingsManager: SettingsManager, presetManager: PresetManager) {
		super(app, plugin);
		this.plugin = plugin;
	...
	private renderFrontmatterMergeSetting(containerEl: HTMLElement): void {
		new Setting(containerEl)
			...
			.setDesc('启用后，插入模板时会自动合并模板与笔记的 frontmatter。模板中的字段会覆盖笔记中的同名字段，tags 字段会智能合并去重。需要安装 yaml 库。')
			.addToggle(toggle => toggle
	...
	private renderFrontmatterPresetsManager(containerEl: HTMLElement): void {
		...
		const actionsContainer = containerEl.createDiv('note-architect-preset-actions');
		...
		const presetsListContainer = containerEl.createDiv('note-architect-presets-list');
		...
		renderPresetListUI({
			...
			callbacks: {
				onRename: async (preset, newName, context) => {
					await this.handlePresetRename(preset, newName, refreshPresetsList, context);
				},
	...
	private async handlePresetRename(
		...
		if (!trimmedName) {
			context.nameInputEl.value = preset.name;
		...
		const modal = new RenamePresetModal(this.app, {
			...
			newName: trimmedName,
			suggestedId,
			validateId: (candidate, currentId) =>
	...
	private async migratePresetBindings(
		...
		const templateFiles = vault
			...
			.filter(
				(file) =>
					file.extension === 'md' &&
					this.isInsideTemplateFolder(file.path, folderPath),
			);

		const updatedRecords: Array<{ file: TFile; previousContent: string }> = [];
		...
		try {
			for (const file of templateFiles) {
				...
				const currentId =
					typeof parsed.frontmatter[PRESET_CONFIG_KEY] === 'string'
						? (parsed.frontmatter[PRESET_CONFIG_KEY] as string)
						: undefined;
	...
	private isInsideTemplateFolder(filePath: string, folderPath: string): boolean {
